...
Take user input — the user types their favorite genres into the terminal (comma-separated)
Vectorize preferences and items — genres are converted into multi-hot vectors
Match preferences using similarity — cosine similarity scores each movie against the user's profile
Display recommended items — the top 5 highest-scoring movies are shown, ranked by match score
...
FileDescriptionproject3_ai_recommendation.pyMain script (Jupyter-style, interactive terminal input)
...
bashpython project3_ai_recommendation.py
